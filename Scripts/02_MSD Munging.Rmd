...
msd_format_long <-
  ...
  mutate(FY=ifelse(fiscal_year %in% 2020, "FY20", "FY19"))%>%
  #str_remove_all(c("Q"))
%>%
 # tostring(c("FY", "Q"))
  mutate(period= paste(FY,Q))
  #separate(Qtr, into=c(NA,"Q"), sep="([r])") %>%
view(msd_format_long)
glimpse(msd_format_long)
...
# 
# 
# msd_format_wide1 <-msd_format_long%>%
#   select(!fiscal_year)%>%
#   spread(indicator, Q) %>%
#   rename(country=countryname, age_group=ageasentered)%>%  
#   pivot_wider(
#      names_from=FY,
...
#   )  %>%
# 
# view(msd_format_wide1)
#   pivot_wider(names_from = c(FY, Q),
#               names_sep = "Q") 
# 
# %>%
...
msd_format_wide <- msd_format %>%
  #long to wide so that tx_Curr and TX_new are on same row for country and year and age
  spread(indicator, qtr1, qtr2, qtr3, qtr4) %>%
